...
GitHub Issue / Documentation Suggestion

## Suggested Title

Docs: add beginner FAQ for Moss agent workflow and safety boundaries

## Issue Body

## Summary

I would like to suggest adding a beginner-friendly FAQ that explains Moss's agent workflow and safety boundaries in plain language.

I noticed that Moss already has strong technical docs around the `discover → load → action → simulate` workflow, MCP tool contracts, and protocol onboarding. As a new builder reading the project, I found the core idea very useful: Moss helps AI agents construct and simulate unsigned transactions, while keeping signing and sending outside the agent's responsibility.

A short FAQ could help new contributors and AI/Web3 beginners understand this model faster.

## Suggested FAQ topics

-   What problem does Moss solve for AI agents?
    
-   What is a Capability in Moss?
    
-   What do `discover`, `load`, `action`, and `simulate` each do?
    
-   Why does Moss construct and simulate unsigned transactions instead of signing or sending them?
    
-   What is the role of a Receipt?
    
-   What are Warnings used for?
    
-   How should a wallet or user review Moss output before signing?
    
-   How is Moss different from a normal SDK or direct contract call helper?
    

## Why this might help

For new builders, the hardest part is not only running the code, but understanding the safety model. AI agents can help prepare and explain onchain actions, but they should not blindly execute user funds or sign transactions.

A beginner FAQ would make this boundary clearer and could support:
...
-   Monad ecosystem developers exploring protocol integrations
    
-   non-core contributors who want to help with docs/tutorials
    

## Possible location

This could be added as one of:

-   `docs/beginner-faq.md`
    
-   a new section in the README
    
-   a companion doc linked from Getting Started
    

## Related docs/issues

This idea is related to the existing beginner and workflow documentation direction, but focuses specifically on FAQ-style explanations and safety boundaries for new AI/Web3 builders.

I would be happy to draft a first version if maintainers think this direction is useful.
...
我选择的 Builder 身份是：**Ops Builder / Product Ops Builder**。

我目前更擅长的不是直接开发新的 Adapter 或修复杂 Bug，而是阅读项目、理解用户路径、整理文档、优化新手体验，并把复杂概念转化成更容易理解的教程、FAQ 或学习笔记。因此我希望从文档和开发者体验入手，为 Moss 做轻量但有价值的开源贡献。

## 2\. 贡献方向

我的贡献方向是：**完善 Moss 的新手入门材料和 Agent Workflow 说明**。

Moss 的 README 已经说明它通过 `discover → load → action → simulate` 把 Monad 协议交互变成 Agent-callable Capabilities，并且只构建和模拟未签名交易，不负责签名和发送。但对刚接触 AI Agent + Web3 的新手来说，这套流程还是比较抽象。

我希望贡献一份更偏新手友好的文档，比如：

-   Beginner-friendly Quick Start 中文笔记
    
-   Agent Workflow 图解说明
    
-   Moss 常见问题 FAQ
    
-   “为什么 Moss 不签名、不发送交易”的解释
    
-   `discover / load / action / simulate` 四步流程示例说明
...
第一，我自己这几周也在学习 Monad、AI Agent 和 Web3 开源项目，能比较清楚地感受到新手卡在哪里。比如很多人可能知道 AI Agent 可以调用工具，但不理解为什么链上操作需要模拟、Receipt、Warnings 和用户签名分离。

第二，Moss 这类项目的价值不只在代码，也在于让更多开发者理解它的安全边界。如果文档能讲清楚“Agent 可以构建和模拟交易，但不能替用户签名”，会更容易降低误用风险。

第三，我的 Week 2 主方向是 Ops / Product Ops，适合承担文档、教程、传播和开发者体验优化工作。这类贡献虽然不是核心代码，但能帮助项目吸引更多 Builder。

## 4\. 本周目标

本周我计划完成以下目标：

1.  阅读 Moss README、Getting Started、MCP tool contracts、Agent safety rules。
    
2.  整理 Moss 的项目结构和核心概念。
    
3.  输出一份中文学习笔记，解释 Moss 解决什么问题。
    
4.  写一份新手 FAQ 初稿。
    
5.  选择一个最小贡献点，准备提交 Issue 或 Pull Request。
...
-   一份 `Moss Beginner Notes.zh-CN.md`
    
-   一份 `Moss FAQ Draft.md`
    
-   一张 Agent Workflow 流程说明：  
    `discover → load → action → simulate → user reviews → wallet signs`
    
-   一个 GitHub Issue：  
    建议补充 Beginner-friendly 中文 Quick Start 或 FAQ
    
-   如果时间允许，提交一个文档类 Pull Request
...
阅读 README、docs 和 examples，记录项目结构、核心术语和不理解的地方。

### Day 2：整理 Agent Workflow

重点理解 `discover / load / action / simulate` 四步分别解决什么问题，并用新手能理解的语言改写。

### Day 3：整理 FAQ

围绕新手常见问题写 FAQ，例如：

-   Moss 是 SDK 还是 Agent 框架？
    
-   Moss 为什么不签名和发送交易？
    
-   Receipt 和 Warnings 有什么作用？
...
确认自己的文档没有误解 Moss 的安全边界，不把模拟说成真实发送交易，不把 Agent 能力描述得过度自动化。

### Day 5：准备开源贡献

整理成 Markdown 文档，并在 GitHub 上提交 Issue 或 PR。如果暂时不确定是否适合直接 PR，可以先开 Issue 说明想补充 Beginner FAQ，并附上文档草稿。

## 7\. 我能提供的 Proof of Work

我已经完成过 Moss 的 GitHub Exploration Log，阅读了 README、Docs、Issues、Pull Requests，并整理了项目结构、核心问题和我感兴趣的 Issue / PR。

我也完成过 Monad Rank Rush Campaign Kit，说明我可以把 Web3 项目从技术概念整理成更容易理解的产品文档、活动方案和用户路径。

已有作品链接：

-   Monad Rank Rush：[https://github.com/euphoria17721-tech/monad-rank-rush](https://github.com/euphoria17721-tech/monad-rank-rush)
    
-   Moss 项目：[https://github.com/nishuzumi/moss](https://github.com/nishuzumi/moss)
    

## 8\. 一句话总结

我希望作为 Ops Builder，从文档、教程和开发者体验切入，为 Moss 补充更适合新手理解的 Agent Workflow 和 FAQ，让更多 Builder 能安全、准确地理解 Moss 如何帮助 AI Agent 调用 Monad 协议能力。
...
Moss 新人入门指南：理解 AI Agent 如何安全参与 Web3
项目链接：<https://github.com/nishuzumi/moss>

1. Moss 是什么？
   Moss 是一个面向 Monad 生态的 AI Agent × Web3 开源框架。
   它的核心目标是：
   让 AI Agent 能够理解、构建和模拟链上协议操作，但不直接替用户签名或发送交易。
   Moss 的 README 中写到，它通过：
   discover → load → action → simulate
   把 Monad 协议交互变成 Agent 可以调用的 Capabilities。它会构建和验证 未签名交易，但不会签名，也不会发送交易。
   这对 Web3 很重要，因为链上操作涉及钱包、签名、授权和资产安全。AI Agent 可以帮助用户理解和准备操作，但最终签名权应该留给用户。
2. 为什么 AI Agent 需要 Moss？
   在普通应用里，AI Agent 可以帮用户点击按钮、整理资料、调用 API。
   但在 Web3 里，情况更复杂。
   一笔链上操作可能涉及：
   钱包地址
   Token 授权
   ...
   交易失败或滑点风险
   不可逆状态变化
   如果 Agent 直接根据一句自然语言生成交易并发送，风险会很高。
   Moss 的价值在于，它给 Agent 增加了一个更安全的中间层：
   先发现可以调用什么能力。
   再读取这个能力需要什么参数。
   然后构建未签名交易。
   最后模拟结果，让用户在签名前理解可能发生什么。
   所以 Moss 不是让 Agent “更大胆地自动执行”，而是让 Agent “更清楚、更安全地辅助用户”。
3. Moss 的核心工作流
   Step 1：discover
   discover 的作用是发现当前有哪些协议能力可以调用。
   你可以把它理解成：
   Agent 先问系统：“我现在能操作哪些协议？有哪些方法可以用？”
   比如：
   ERC-20 transfer
   ERC-20 approve
   WMON wrap / unwrap
   Kuru swap
   PancakeSwap swap
   这一步帮助 Agent 避免凭空猜测协议能力。
   Step 2：load
   load 的作用是读取某个 Capability 的详细规则。
   你可以理解成：
   Agent 选中一个能力后，需要先看说明书。
   比如一个 swap 能力可能需要：
   tokenIn
   tokenOut
   amountIn
   slippage
   account
   这一步能让 Agent 明确参数要求，而不是随便拼交易。
   Step 3：action
   action 的作用是根据参数构建操作。
   这里 Moss 会生成未签名交易。
   注意：这一步还没有真正发送交易，也没有动用用户资产。
   它只是把用户意图转化成一笔可以进一步检查和模拟的链上操作。
   Step 4：simulate
   simulate 的作用是在签名前模拟交易结果。
   这一步非常关键。
   模拟结果可以告诉 Agent 和用户：
   交易是否可能失败
   会产生哪些状态变化
   是否有 Warnings
   Receipt 是否能解释所有 Changes
   如果模拟失败，或者出现风险警告，就不应该继续让用户签名。
4. Moss 最重要的安全边界
   Moss 有一个非常重要的设计：
   Moss 只构建和模拟未签名交易，不负责签名和发送交易。
   也就是说：
   Agent 可以帮助用户理解操作。
   Agent 可以准备交易。
   Agent 可以模拟交易结果。
   Agent 可以解释风险。
   但最终是否签名，要由用户或钱包决定。
   这对 AI Agent × Web3 很重要。
   因为如果 Agent 直接拥有签名和发送权限，用户可能在不理解交易内容的情况下损失资产。Moss 把 Agent 辅助和用户授权分开，能让链上交互更可解释、更可验证。
5. 新人如何阅读 Moss 项目？
   如果你是第一次打开 Moss 仓库，可以按这个顺序阅读：
6. 先读 README
   README 会告诉你：
   Moss 是什么
   为什么需要 Moss
   ...
   如何作为 library 使用
   项目目录结构是什么
7. 再读 Getting Started
   Getting Started 更适合实际上手。
   它会帮助你理解 Moss 的基本运行流程。
8. 再看 docs
   重点关注：
   MCP tool contracts
   Agent safety rules
   ...
   这些文档能帮助你理解 Moss 不只是一个 SDK，而是一套面向 Agent 的链上交互框架。
9. 最后看 examples
   examples 可以帮助你理解代码如何使用 Moss。
   如果你暂时看不懂全部代码，也可以先理解输入、输出和流程。
10. 新人可以如何参与 Moss 开源贡献？
    不一定要一开始就写复杂代码。
    Moss 这类项目也很需要文档、教程和 FAQ。
    适合新人的贡献方式包括：
    阅读 README 后整理中文学习笔记。
    写一份 Beginner FAQ。
    解释 discover → load → action → simulate 工作流。
    整理 Moss 和普通 SDK 的区别。
    提出文档改进 Issue。
    帮助补充新手教程。
    记录自己运行 examples 时遇到的问题。
    如果你是 Dev Builder，可以尝试：
    跑通本地环境。
    阅读 packages 目录。
    理解已有协议 Adapter。
    修复文档或示例中的小问题。
    后续尝试贡献新的协议 Adapter。
    如果你是 Ops / Research Builder，可以尝试：
    写教程。
    做项目解读。
    整理使用场景。
    设计新人活动。
...
11. Moss 常见问题 FAQ
    Q1：Moss 是钱包吗？
    不是。
    Moss 不负责保管资产，也不负责替用户签名。
    Q2：Moss 会自动发送交易吗？
    不会。
    Moss 构建和模拟未签名交易，但不签名、不发送。
    Q3：Moss 和普通 SDK 有什么区别？
    普通 SDK 更偏开发者直接调用代码。
    Moss 更强调 Agent 工作流、安全边界、模拟结果和 Receipt 验证，让 AI Agent 可以更可靠地理解链上操作。
    Q4：为什么需要 simulate？
    因为链上操作有风险。
    在用户签名前先模拟，可以提前发现失败、Warnings 或不符合预期的状态变化。
    Q5：什么是 Capability？
    Capability 可以理解成一个协议能力。
    比如 transfer、approve、swap、wrap 都可以是 Agent 能调用的能力。
    Q6：什么是 Receipt？
    Receipt 是模拟后对结果的结构化解释。
    它帮助 Agent 和用户理解这笔操作会产生什么变化。
    Q7：新人适合从哪里开始贡献？
    最适合从文档开始。
    比如整理 FAQ、写中文入门笔记、补充教程、提出 Issue，都是有价值的开源贡献。
12. 一个简单理解
    如果用一句话解释 Moss：
    Moss 是 AI Agent 和链上协议之间的安全协作层。
    它不是让 AI 直接替用户操作钱包，而是让 AI 在签名前帮助用户看懂：
    能做什么
    需要什么参数
    会构建什么交易
    模拟后会发生什么
    有没有风险提示
    这也是我认为 Moss 未来有价值的地方。
    随着 AI Agent 进入 DeFi、钱包、链上任务和多协议交互场景，类似 Moss 这样的框架会越来越重要。
13. 总结
    Moss 的核心不是“让 AI 更自动”，而是“让 AI 更安全地协助用户”。
    它通过 discover → load → action → simulate 的流程，让 Agent 能够标准化地理解和准备链上操作，同时保留用户最终签名权。
...
# Ops Builder 项目检查：准备寻找测试用户

## 选择方向

**Ops Builder**

我从运营和用户测试角度检查项目，重点判断这个项目是否能让新人听懂、是否能找到测试用户、是否能设计出清楚的测试任务。

## 1. 一句话项目介绍

**Monad Rank Rush 是一个基于 Monad 的 30 秒链上社交小游戏，用户可以挑战好友、刷新排行榜、领取徽章，并生成可分享的 Meme 战绩图。**

## 2. 可以从哪里找到 3 名可能使用它的人

### 渠道 1：Monad / Web3 学习群

可以在 Monad 训练营、Web3 Builder 群、学习打卡群里找 1–2 名测试者。

这类用户对 Monad、测试网、钱包和链上任务有基础兴趣，更容易理解项目背景。

### 渠道 2：喜欢小游戏和排行榜的同学

可以从身边喜欢小游戏、积分榜、打卡挑战、社群活动的同学中找 1 名测试者。

他们不一定懂 Web3，但可以帮助判断玩法本身是否足够直观。

### 渠道 3：做 Ops / 内容 / 社区方向的同学

可以找 1 名做运营或内容方向的同学，测试 Meme 战绩图、排行榜机制和分享动机是否成立。

他们能从传播角度判断这个产品有没有社群扩散潜力。

## 3. 用户测试任务

请测试者完成以下任务：

1. 阅读 Monad Rank Rush 的一句话介绍和 Landing Page 草稿。
2. 想象自己正在参与一次 30 秒挑战。
3. 按顺序理解流程：连接钱包 → 开始挑战 → 获得分数 → 查看排行榜 → 领取徽章 → 分享 Meme 战绩图。
4. 回答以下问题：
   1. 你是否能在 1 分钟内理解这个项目是什么？
   2. 你最感兴趣的是排行榜、徽章、好友挑战，还是 Meme 战绩图？
   3. 如果你被好友超过排名，你会不会想再玩一次？
   4. 你觉得这个产品哪里还不清楚？

## 4. 100 字检查结果说明

这次从 Ops 角度检查后，我认为团队下一步不应该先做复杂链上功能，而应该先验证用户是否能快速理解玩法，以及排行榜、徽章和 Meme 战绩图是否真的能激发分享和复玩。接下来团队可以先找 3 名测试者做轻量访谈，用 Landing Page 草稿和流程说明测试项目表达是否清楚，再决定 Dev 侧优先做哪些功能。
...
这周的学习重点从“理解项目”逐渐转向“如何把项目讲清楚，并准备给真实用户测试”。前面我已经围绕 Monad 和 Moss 做了不少学习，包括阅读开源项目、理解 AI Agent × Web3 的安全边界、整理教程、设计活动和运营物料。这周我更明确地站在 **Ops / Product Ops** 的角度，思考一个项目要进入团队协作和用户测试前，需要准备哪些基础内容。

我这周主要围绕 **Monad Rank Rush** 做项目介绍和用户测试准备。这个项目的核心想法是：做一个基于 Monad 的 30 秒链上社交小游戏，用户可以挑战好友、刷新排行榜、领取徽章，并生成可分享的 Meme 战绩图。它不是一个复杂 DeFi 产品，而是一个更偏 Consumer Crypto 的轻量互动场景。我想验证的是：用户能不能快速理解这个玩法，以及排行榜、徽章、好友反超和 Meme 图是否真的能激发参与和分享。

这周一个重要收获是：项目介绍不能只写给自己看，也不能只写给懂技术的人看。真正要做用户测试时，介绍必须足够清楚，让新人 1 分钟内知道“这是什么、怎么玩、为什么值得试”。所以我整理了一句话项目介绍、100–200 字项目简介、测试邀请文案、测试任务、反馈问题和 Landing Page 草稿。这些内容的目的不是包装项目，而是降低用户理解门槛。

我也继续加深了对 Ops 的理解。以前我会觉得运营主要是宣传、发文案、做活动，但现在我觉得 Ops 更重要的是设计用户路径。比如用户第一次看到项目时是否能理解，是否知道下一步该做什么，测试时能不能完成任务，反馈问题是否能帮助团队判断方向。这些都会直接影响团队下一步开发什么、砍掉什么、保留什么。

从项目检查角度看，我认为团队下一步不应该急着做复杂链上功能，而应该先做轻量用户测试。比如找 3 个可能用户：一个 Monad / Web3 学习群里的用户，一个喜欢小游戏和排行榜的普通同学，一个做 Ops 或社区内容的同学。让他们看项目介绍和流程草稿，判断是否能理解、是否有兴趣、是否愿意分享、哪些地方不清楚。这样比直接开发完整功能更节省时间，也能避免做出用户看不懂的东西。

AI 在这周帮我做了很多结构化整理，比如把项目想法转成测试文案、反馈问题、活动方案和 Case Study。但最终方向判断还是需要我自己完成：选择 Ops 作为主方向、判断目标用户是新人、决定先验证表达和传播动机，而不是先做复杂功能。

这周最大的学习收获是：**一个项目能不能继续推进，不只取决于它能不能做出来，也取决于用户能不能听懂、愿不愿意试、会不会给出真实反馈。** 进入 Week 3 后，我希望继续承担 Ops / Product Ops 角色，负责项目介绍、用户测试、反馈整理、活动策划和内容表达，帮助团队把技术想法转化成用户能够理解和参与的产品体验。
...
这几天的学习让我感觉，自己对 Builder 的理解又往前走了一步。前面我更多是在完成一个个任务：写项目介绍、整理活动策划、做 Moss 教程、准备用户测试材料。但现在回头看，这些内容其实都在训练同一件事：**如何把一个模糊的 Web3 想法，慢慢变成别人能理解、愿意参与、可以反馈的东西。**

一开始我会觉得项目最重要的是“有没有技术亮点”或者“能不能做出来”。比如 Monad Rank Rush 这个方向，我最早关注的是它和 Monad 高频交互的关系：小游戏、排行榜、徽章、Meme 战绩图这些功能，听起来是不是适合 Monad。后来我慢慢意识到，项目能不能继续推进，不只是看它理论上能不能做，而是要看用户能不能在很短时间内明白它是什么。

所以这周我开始把重点放在用户测试准备上。比如一句话项目介绍、100–200 字简介、测试邀请文案、用户测试任务和反馈问题。这些看起来不像“核心产品功能”，但其实非常关键。如果连一句话都讲不清楚，用户就很难愿意继续了解；如果测试任务设计得太模糊，用户反馈也会很散；如果反馈问题问得太大，团队就不知道下一步应该改哪里。

我这周最大的收获是：**Ops 不是把项目包装得更好看，而是帮助团队更快知道项目有没有被理解、有没有吸引力、有没有继续做的价值。** 这和单纯宣传不一样。宣传是把信息发出去，而用户测试是把问题收回来。对早期项目来说，能不能收回真实反馈，可能比做更多页面和功能更重要。

我也更清楚自己在团队里能做什么了。我不一定是最适合写合约或做复杂前端的人，但我可以帮助团队把技术想法翻译成用户语言。我可以整理项目介绍、设计测试路径、准备问题、写活动文案、做 FAQ、记录用户反馈，再把反馈转化成下一步产品判断。这个角色对早期 Builder 团队其实很重要，因为很多项目不是输在没人写代码，而是输在用户根本不知道为什么要用。

这周我还继续学习 Moss。Moss 给我的启发是，AI Agent × Web3 项目尤其需要把“边界”讲清楚。AI 可以帮助用户理解协议、构建操作、模拟交易，但不能直接替用户签名和发送交易。这个逻辑也影响了我对项目表达的理解：一个好的项目介绍，不应该只说自己能做什么，也要说清楚自己不做什么，边界在哪里，用户为什么可以信任它。

接下来我希望继续保持 Ops / Product Ops 方向，把自己的工作从“写内容”推进到“帮助团队做判断”。比如在用户测试后，我想整理出：哪些地方用户看懂了，哪些地方没看懂，哪个功能最有吸引力，哪个设定只是我们自己的想象。这样团队就可以更清楚地决定，下一步是先做 Demo、先改介绍、还是先砍掉某个不成立的功能。

这段时间最大的变化是，我开始不再只把自己当成一个“交作业的人”，而是开始像一个真正的 Builder 一样思考：这个东西为什么要存在？谁会用？他们能不能理解？他们下一步会不会行动？如果这些问题答不上来，再多功能也可能只是自嗨。
