...
Moss定义了一个特殊标识`NATIVE`，表示这里不是某个 ERC-20合约地址，而是Monad原生资产MON。Moss把 “ERC-20与native MON的通用资产操作” 放在同一个Package里。
...
代码自动判断 `params.token === NATIVE`。

同一个 Capability `erc20.transfer`可以处理两类资产：
