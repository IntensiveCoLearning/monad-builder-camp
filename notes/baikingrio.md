...
我重点核验了 Monad 的实际链上状态：

\- Monad Testnet 的 Chainlink Verifier 文档地址是 0xC539169910DE08D237Df0d73BcDa9074c787A4a1；

\- 但通过 Testnet RPC 实测，该地址当前没有 bytecode，无法形成链上验证闭环；

\- Monad Mainnet 的 Verifier 地址是 0xEd813D895457907399E41D36Ec0bE103E32148c8，实测存在合约代码，因此 Mainnet 技术路径成立；

\- Data Streams 需要付费订阅和 HMAC API 凭据，不能将凭据暴露给前端或提交到仓库。

这也让我更明确了 BTC/USD 15 分钟预测 Round 的 Oracle 规则：开盘和结算不能只读“当前价格”，而要验证 report 的 feedId、观察时间、过期时间、正价格和指定时间窗口。没有合格报告时，应取消 Round 并退款。

当前 Monad Testnet 阶段，我会继续使用已经真实跑通的 Pyth Pull Oracle；它已完成签名更新、链上验证和价格记录的真实交易闭环。Chainlink Data Streams 更适合作为项目进入 Monad Mainnet 后的低延迟 Oracle 备选方案。

今日学习记录：[https://github.com/baikingrio/monad-builder-camp/blob/main/daily/2026-07-26.md](https://github.com/baikingrio/monad-builder-camp/blob/main/daily/2026-07-26.md)
...
这条流程已经在 Monad Testnet 拿到真实链上回执：一次是启用 Roles，另一次是 Session Key 发起的免弹窗抽奖。对非 draw() 的越权调用也已实际 revert。

今天还同步修复了前端里旧 legacy-session 文案对应的两条测试断言。现在 npm test 已全绿：29 个测试文件、178 个测试全部通过；之前的 Foundry 权限边界测试也保持 55 passed。

今天最大的收获是：免弹窗不是跳过授权，而是把授权集中为一次可见、可检查的操作，再用角色权限把后续能力收敛到最小范围。当前 Demo 仅运行在 Monad Testnet，不涉及真实资产。

今日学习记录：[https://github.com/baikingrio/monad-builder-camp/blob/main/daily/2026-07-24.md](https://github.com/baikingrio/monad-builder-camp/blob/main/daily/2026-07-24.md)
...
我把 Safe 7579 和 Smart Sessions 的上游版本、合约 artifact 哈希、允许调用的 Lucky Draw 地址、draw() selector 和 value = 0 固定成可检查的 manifest，并增加了测试 gate：依赖版本不是精确版本、关键策略依赖缺失，或有人误把部署状态改成可用，验证都会失败。

今天最大的收获是：Session Key 的安全不能只靠服务端限流。指定合约和指定方法、零 value、有效期、撤销机制和 UserOperation 验证，都应该有链上或可复查的边界。

当前我确认了一个重要限制：Smart Sessions 的 TimeFrame / ValueLimit Policy 在固定上游版本中只有 mock 来源，不能直接用于真实部署；已发布的 UniActionPolicy 也不适配无参数的 draw() 调用。

所以今天没有为了“跑通”而接入未经审查的策略，也没有创建新 Safe 或广播交易。下一步会继续寻找有审计与部署溯源的策略；在此之前，旧的测试网 Demo 边界保持不变。

今日学习记录：[https://github.com/baikingrio/monad-builder-camp/blob/main/daily/2026-07-23.md](https://github.com/baikingrio/monad-builder-camp/blob/main/daily/2026-07-23.md)
...
今天最重要的收获是：Sponsor 只解决 gas，不等于跳过授权；真正的无感体验仍需要把首次 Owner 授权、Session Key 范围、服务端策略校验和链上回执拆开处理。

今日学习记录：[https://github.com/baikingrio/monad-builder-camp/blob/main/daily/2026-07-22.md](https://github.com/baikingrio/monad-builder-camp/blob/main/daily/2026-07-22.md)
...
今天同步并核对了 OddLane 最新的 AA 与 Demo 更新。ZeroDev Kernel 这条路线已经不只是文档调研：我确认了独立交易账户在 Monad Testnet 上完成了 Owner UserOperation、领取 Test USDC、存入订单簿，以及由 Owner 提现回钱包的真实资金闭环。

 

这次验证让我更明确地区分了 Owner 和交易账户的职责。用户身份由 Owner 保持控制，但 Test USDC、订单簿可用现金、订单和持仓统一归属 Kernel 交易账户，这样才能支持后续“一次启用交易账户、受限操作快速执行”的产品体验。

<br />

Session Gas 赞助目前仍未通过连续成功 Gate，不能宣称 OddLane 已经稳定免 Gas。现阶段会保留清晰的降级：赞助不可用时提示用户自付 Gas 或改用 Owner 确认，而不是让交易静默失败。下一步是补 Session 下单、撤单、赎回、越权拒绝、过期和撤销的真实 Testnet E2E。
