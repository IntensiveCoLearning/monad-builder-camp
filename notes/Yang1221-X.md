...
# Day 5 学习笔记｜Monad 理解、Week 1 复盘与方向选择

## 一、Monad 核心理解：不只是”更快”，而是产品范式的改变

### 认知升级

之前以为 Monad 就是”快一点的以太坊”，现在想明白了——速度的量变会带来产品形态的质变。就像 2G 到 4G 不是网快了点，而是催生了短视频、直播、外卖这些之前根本不存在的东西。

Monad 的 10,000 TPS + 亚秒级出块，意义不在于”让 Uniswap 交易更快”，而在于”让一类之前链上根本没法玩的产品变成可能”。

### 开发者视角的核心差异

| 维度 | 以太坊 | Monad | 对产品的影响 |
| 出块时间 | ~12 秒 | ~1 秒 | 用户交互后不用等，体验接近 Web2 |
| TPS | ~15-20 | ~10,000 | 百人同时用一个 Dapp 也不堵 |
| Gas 费 | 波动大，拥堵时天价 | 低且稳定 | 微交互的成本可以忽略 |
| 执行方式 | 串行 | 并行 | 互不影响的交易一起跑，性能指数级提升 |
| EVM 兼容 | - | 完全兼容 | Solidity 直接部署，学习成本为零 |

**关键判断：** 之前链上产品只能做”低频大额”（转账、质押、Swap），Monad 让”高频小额/零额”产品（游戏、社交、内容互动）成为可能。

## 二、Week 1 Build Log 整理

### 1\. 钱包 & 交易

•         小狐狸直接加 Monad 测试网，配置和加其他 EVM 链一模一样

•         第一笔转账：点确认后几乎立刻看到成功，体感和以太坊等 12 秒完全不同

•         区块浏览器界面和 Etherscan 几乎一样，上手零成本

### 2\. 智能合约

•         用 Solidity 写了个简单的存证合约，直接部署到 Monad 测试网，代码一行没改

•         调用写操作 ~1 秒出结果，读操作和以太坊一样瞬间返回

•         Gas 费低到可以忽略，部署一个简单合约只花了约 0.001 MON

### 3\. AI 协作开发

•         AI 写 Solidity 初版代码靠谱，但复杂逻辑和权限控制要自己审

•         最佳用法：说清楚需求 → AI 出初版 → 人审核修改 → 部署测试

•         AI 对底层原理（比如并行执行）的解释有时候不准确，需要交叉验证

### 4\. 安全意识

•         EVM 兼容意味着经典漏洞（重入、溢出、权限问题）在 Monad 上一样存在

•         高性能可能带来新攻击面（比如利用并行执行的竞态条件）

•         原则：测试网随便玩，主网上线前必须审计

## 三、Mini Demo 0 总结：链上答题原型跑通了

### 做了什么

一个极简的链上答题 Dapp： - **合约：** 存 5 道题的答案哈希 + 用户得分记录 + 提交答案函数 - **前端：** 显示题目 → 用户选答案 → 钱包签名提交 → 链上判断对错 → 显示得分

### 实测数据和结论

| 指标 | 实际结果 | 预期对比 |
| 合约部署 | 直接部署，代码零修改 | 和预期一致 |
| 单次答题确认时间 | ~0.8-1.2 秒 | 比预期还快，几乎感受不到等待 |
| 单次答题 Gas 费 | 约 0.0002 MON | 低到可以忽略，一百道题才 0.02 个币 |
| 并发测试（10 人同时答） | 全部成功，没有拥堵失败 | 10 笔交易在同一个区块里确认了 |

### 核心发现

1.  **体验真的接近 Web2 了** 答完题点提交，一眨眼就出结果。如果做个乐观更新（点了就显示对/错，后台异步等确认），用户根本感觉不到自己在用区块链产品。
    
2.  **Gas 费不是问题，是优势** 一道题 0.0002 MON，项目方完全可以替用户代付。“零成本使用 Dapp”在以太坊上是天方夜谭，在 Monad 上是标配。
    
3.  **高频交互真的可行** 10 个人同时答题毫无压力，按这个比例推算，百人甚至千人级别的同时在线互动理论上也能撑住。
    

### 不足和优化方向

•         题目内容不应该存在合约里，太费 gas，应该放前端或 IPFS，只存答案哈希

•         可以优化成一次提交所有答案，减少交易次数

•         前端还很粗糙，只有基本功能

## 四、Week 2 方向选择：为什么我坚定选 Ops

### 先说 Tech 为什么不适合我

1.      **技术壁垒没那么高。** 这周跑下来最深的感受是——Monad 对开发者太友好了。Solidity 直接写、工具链全复用、部署方式一模一样。只要会以太坊开发，转 Monad 基本零成本。纯技术方向的差异化不大。

2.      **我对”性能极致优化”兴趣不大。** Tech 方向往深了走，是研究怎么让合约更并行友好、怎么榨干每一分 TPS。这个方向很重要，但不是我最感兴趣的。我更关心的是”用这个性能做什么产品能打动用户”，而不是”怎么把性能提到最高”。

3.      **短期看不到正反馈。** 技术优化是个慢活儿，优化了 10% 的 gas 费，用户根本感知不到。我更喜欢做能快速看到结果、拿到用户反馈的事情。

### 再说 Research 为什么不适合我

1.      **缺基础。** Research 方向需要对共识机制、并行执行、零知识证明这些底层技术有很深的理解，我目前还是入门水平，硬上会很吃力。

2.      **太抽象。** 写一篇分析报告、做一个赛道调研，对不对、好不好，很难验证。我更喜欢做”做完就能看到效果”的事。

3.      **和我的长期方向不匹配。** 我以后想做的是产品/增长/社区方向，不是纯研究。

### 为什么 Ops 是最适合我的方向

**理由 1：这是 Monad 生态当前最缺的东西** 一条新链最缺的是什么？不是开发者（EVM 兼容意味着开发者存量巨大），而是用户和真实的使用场景。 技术再牛，没人用就是空气。Ops 做的事情——拉新、留存、社区传播、用户增长——是任何一条新链生态早期最核心的需求。

**理由 2：能直接验证”高性能到底有没有用”** 很多人说”10,000 TPS 是解决方案找问题”，意思是性能过剩，没人需要这么高的 TPS。 这件事光靠辩论没用，得靠做。我做 Ops，就是要用实际行动回答一个问题：高性能链上的产品，用户到底买不买账？ 如果我做的答题游戏，真的有几百上千人来玩、来传播，那就说明”高频交互 + 区块链”这条路是通的。这比写十篇分析文章都有说服力。

**理由 3：技能可迁移，越做越值钱** Ops 涉及的能力： - 怎么设计用户愿意参与的活动 - 怎么让用户自发传播 - 怎么理解用户需求和痛点 - 怎么用数据判断效果

这些能力不管以后做什么方向（产品、运营、增长、甚至自己创业）都用得上，是底层能力。

**理由 4：符合我的性格和优势** 我喜欢和人打交道，喜欢琢磨”怎么让一件事火起来”，喜欢快速试错快速迭代。Ops 方向每天都在和用户、和社区打交道，每天都有新反馈，对我来说比对着代码调性能有意思得多。
