...
-   **AutoGen**：像拉微信群一样让多个智能体聊天协作
    
-   **AgentScope**：阿里达摩院出的，消息驱动，适合大规模生产
    
-   **CAMEL**：两个智能体角色扮演，互相配合完成任务
    
-   **LangGraph**：用"图"来控制流程，每一步精确可控
    

### 给agent装上“记忆”

智能体天生是"鱼的记忆"——每次对话都是新的，啥也记不住。

**四种记忆**（模仿人脑）：

| 类型 | 人脑对应 | 存什么 | 存哪里 |
| --- | --- | --- | --- |
| 工作记忆 | 短期记忆 | 当前对话 | 内存（用完就忘） |
| 情景记忆 | "上次发生了什么" | 对话历史 | SQLite + 向量数据库 |
| 语义记忆 | "我知道的知识" | 概念和关系 | Neo4j 图数据库 |
| 感知记忆 | 多感官信息 | 图片、音频 | 分模态存储 |

**RAG（检索增强生成）**：让智能体先去"知识库"里查资料，再用查到的资料回答，大幅减少胡说八道。

高级技巧：

-   **MQE**：同一个问题换几种问法去搜，召回率提高 30%-50%
    
-   **HyDE**：先假设一个答案，用这个"假答案"去搜真资料
    

### 管理好上下文

上下文是智能体的**有限资源**。塞进去的东西越多，模型反而越容易搞混——这叫"上下文腐蚀"。
...
2.  **Select**（筛选）：算相关性分数，只留最相关的，填满 token 预算
    
3.  **Structure**（组织）：按固定模板排列，不乱
    
4.  **Compress**（压缩）：超了上限就压缩，保结构不保字数
